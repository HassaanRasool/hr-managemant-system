...
import { jobStatusVariants } from "@/data/jobs";
...
export function JobPostingsTable({ searchTerm }: JobPostingsTableProps) {
  const [mounted, setMounted] = useState(false);
  const { user } = useAuth();
  const isAdmin = user?.role === "admin";
  const jobs = useAppStore((state) => state.jobs) as Job[];
  ...
  const filteredJobs = jobs.filter((job) => {
    // Add a fallback to an empty string for searchTerm
